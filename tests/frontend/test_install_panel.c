...
static int find(const mh_ui_row *r, unsigned n, const char *l) {
    for (unsigned i=0;i<n;i++) if (r[i].label && !strcmp(r[i].label,l)) return r[i].id;
    return -1;
}
/* A board that can take a package, and one that cannot. */
static mh_pclink *taken;
static struct mh_pclink *fake_install(void *b, const char *path) {
    (void)b; taken = mh_pclink_open(path); return taken;
}
...
int main(void) {
    char dir[512];
    snprintf(dir, sizeof(dir), "%s/mh-install-XXXXXX", mh_temp_dir());
    if (!mh_mkdtemp(dir)) return 1;
    ...
    mh_ui *ui=NULL; mh_ui_open(&ui,r,w);
    mh_ui_set_buttons(ui, 1u<<MH_UI_ICON_INSTALL);
    mh_setenv("HOME", dir);

    /* A machine with no link says so rather than offering a dead picker. */
    mh_runtime_ops bare = {0};
    mh_runtime nolink = { .ops = &bare };
    mh_install_panel_open(ui, &nolink);
    const mh_ui_row *rows = mh_ui_panel_rows(ui);
    unsigned n = mh_ui_panel_row_count(ui);
    int choose = find(rows,n,"Choose a package\xe2\x80\xa6");
    CHECK(choose >= 0);
    CHECK(mh_install_panel_row(ui,&nolink,choose));
    rows = mh_ui_panel_rows(ui); n = mh_ui_panel_row_count(ui);
    CHECK(rows[0].kind == MH_UI_ROW_HEADING);
    CHECK(rows[0].label && strstr(rows[0].label,"no link"));
    mh_install_panel_close(ui);

    /* A machine that can: choose a package and the transfer starts. */
    mh_runtime_ops ops = { .install = fake_install };
    mh_runtime m = { .ops = &ops };
    mh_install_panel_open(ui, &m);
    rows = mh_ui_panel_rows(ui); n = mh_ui_panel_row_count(ui);
    choose = find(rows,n,"Choose a package\xe2\x80\xa6");
    CHECK(choose >= 0);
    CHECK(mh_install_panel_row(ui,&m,choose));
    rows = mh_ui_panel_rows(ui); n = mh_ui_panel_row_count(ui);
    ...
    CHECK(mh_install_panel_row(ui,&m,pkgrow));
    rows = mh_ui_panel_rows(ui); n = mh_ui_panel_row_count(ui);
    CHECK(taken != NULL);
    CHECK(rows[0].kind == MH_UI_ROW_HEADING);
    ...
    mh_install_panel_close(ui);
    if (taken) mh_pclink_close(taken);
    mh_ui_close(ui);
    SDL_Quit();
    char clean[300];
