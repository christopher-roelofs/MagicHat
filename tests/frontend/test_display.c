...
{
    char out[64];
    CHECK(mrc_state_path_for("roms/PIC-2000.rom",out,sizeof(out)) &&
          !strcmp(out,"roms/PIC-2000.state"));
    /* No extension at all, and a dot in a directory along the way is not one. */
    CHECK(mrc_state_path_for("roms/PIC2000",out,sizeof(out)) &&
          !strcmp(out,"roms/PIC2000.state"));
    CHECK(mrc_state_path_for("a.b/rom",out,sizeof(out)) &&
          !strcmp(out,"a.b/rom.state"));
    /* Launching a device from its own state would save over it with a
     * machine that never booted. */
    CHECK(!mrc_state_path_for("roms/PIC-2000.state",out,sizeof(out)));
    CHECK(!mrc_state_path_for("roms/PIC-2000.rom",out,8));
    return 0;
}
...
int main(void)
{
    ...
    mrc_sdl_display d;
    CHECK(mrc_sdl_display_open(&d,"display test",4,4,24));
    CHECK(mrc_sdl_display_resize(&d,2,2));
    const uint8_t levels[]={0,1,2,3}, gray[]={255,170,85,0};
    uint32_t expected[4];
    mrc_sdl_display_gray(expected,gray,4,MRC_FRAME_GRAY8);
    mrc_sdl_display_gray(d.pixels,levels,4,MRC_FRAME_LCD2);
    for(unsigned i=0;i<4;i++) CHECK(d.pixels[i]==expected[i]);
    CHECK(expected[0]==0xffffffff && expected[3]==0xff000000);
    CHECK(mrc_sdl_display_present(&d,2,2,false));
    uint32_t screen[16];
    ...
    CHECK(mrc_sdl_display_resize(&d,3,2));
    ...
    for(unsigned r=0;r<4;r++) {
        SDL_setenv("MRC_DISPLAY_ROTATION",rotations[r],1);
        CHECK(mrc_sdl_display_present(&d,3,2,false));
        uint32_t portrait[12*18];
        CHECK(SDL_RenderReadPixels(d.renderer,NULL,SDL_PIXELFORMAT_ARGB8888,
                                   portrait,12*4)==0);
        SDL_Rect dst;
        mrc_sdl_panel_rect(d.renderer,3,2,false,0,0,&dst);
        for(unsigned y=0;y<2;y++) for(unsigned x=0;x<3;x++) {
            int ox,oy; unsigned px,py;
            mrc_sdl_panel_to_output(&dst,x,y,3,2,&ox,&oy);
            CHECK(portrait[oy*12+ox]==colors[y*3+x]);
            CHECK(mrc_sdl_panel_point(&dst,ox,oy,3,2,&px,&py));
            CHECK(px==x && py==y);
        }
        unsigned px,py;
        CHECK(!mrc_sdl_panel_point(&dst,dst.x-1,dst.y,3,2,&px,&py));
        CHECK(!mrc_sdl_panel_point(&dst,dst.x+dst.w,dst.y,3,2,&px,&py));
    }
     ...
     */
    mrc_runtime_ops ops = {.pen=pen,.save_state=save_state};
    mrc_runtime runtime = {.ops=&ops};
    ...
    CHECK(!mrc_sdl_can_save_state(&runtime));
    mrc_state_set_path("/nonexistent-directory/device.state");
    CHECK(mrc_sdl_can_save_state(&runtime));
    mrc_runtime_ops cannot = {.pen=pen};
    mrc_runtime no_save = {.ops=&cannot};
    CHECK(!mrc_sdl_can_save_state(&no_save));

    Uint64 wall=SDL_GetPerformanceCounter();
    SDL_Event event = {.type=SDL_APP_WILLENTERBACKGROUND};
    CHECK(mrc_sdl_display_event(&d,&event,&runtime,&wall));
    ...
    CHECK(mrc_sdl_display_resize(&d,2,2) && d.pixels==old);
    CHECK(!mrc_sdl_display_resize(&d,UINT32_MAX,2) && d.pixels==old);
    CHECK(!mrc_sdl_display_resize(&d,0,2) && d.pixels==old);
    /* Exercise the LCD cell-size transitions that previously freed twice. */
    for(unsigned i=0;i<30;i++) {
        unsigned size=2+i%5;
        CHECK(mrc_sdl_display_resize(&d,size,size));
        CHECK(d.width==size && d.height==size);
        CHECK(mrc_sdl_display_present(&d,2,2,true));
    }
