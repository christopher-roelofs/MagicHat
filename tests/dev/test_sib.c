...
int main(void)
{
    ...
    mrc_bus_init(&bus);
    CHECK(mrc_bus_add_ram(&bus, "sound", 0, ram, sizeof(ram), sizeof(ram)));
    cpu.bus = &bus;
    ...
    CHECK(mrc_sib_snd_rate_hz(sib) == 8000);
    mrc_sib_pump_audio(sib);
    ...
    mrc_sib_set_audio_sink(sib, sample, NULL);
    ...
    mrc_sib_set_audio_sink(sib, NULL, NULL);
    ...
    CHECK(mrc_sib_output_rate_hz(sib) == 16000);
    sib->dmactrl = 0;
    CHECK(mrc_sib_write(sib, TX39_SIBSF0CTRL,
