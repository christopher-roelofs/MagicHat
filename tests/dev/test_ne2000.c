...
static void wr(ne2000 *n, unsigned port, unsigned value) { mh_ne2000_write(n, port, 1, value); }
...
static void packets(void)
{
    ...
    mh_ne2000_tick(&n, 67199); CHECK(!sent);
    mh_ne2000_tick(&n, 67200); CHECK(sent == 1 && !n.tx_pending && !(n.cr & 4));
    CHECK(n.tsr == 1 && (n.isr & 2) && mh_ne2000_irq(&n));
    wr(&n, 0, 0x22); wr(&n, 7, 2); CHECK(!mh_ne2000_irq(&n));
    n.send = NULL; wr(&n, 0, 0x26); mh_ne2000_tick(&n, 134400);
    CHECK(n.tsr == 0x10 && (n.isr & 8) && !(n.isr & 2));
    wr(&n, 0, 0x26); wr(&n, 0, 0x21);
    CHECK(n.tx_pending && (n.cr & 3) == 3); /* STP preserves STA until restart */
    mh_ne2000_tick(&n, 300000);
...
static void boundary_comparator(void)
{
    ...
    mh_ne2000_init(&n,mac); memcpy(n.par,mac,6);
    ...
    wr(&n,3,0x80);
    CHECK(mh_ne2000_receive(&n,frame,sizeof(frame)) && n.curr==0x46);
    CHECK(mh_ne2000_receive(&n,frame,sizeof(frame)) && n.curr==0x47);
    CHECK(!n.unsupported && !n.rx_overruns);
    n.curr=0x7f; wr(&n,3,0x46);
    CHECK(!mh_ne2000_receive(&n,frame,sizeof(frame)) && n.curr==0x7f);
    CHECK(n.isr&0x10);
    wr(&n,7,0x10);
    ...
    CHECK(mh_ne2000_receive(&n,frame,sizeof(frame)) && n.curr==0x41);
    CHECK(!mh_ne2000_receive(&n,frame,sizeof(frame)) && n.curr==0x41);
}
int main(void)
{
    ...
    mh_ne2000_init(&n, mac);
    CHECK(mh_ne2000_read(&n, 0, 1) == 0x21);
    wr(&n, 7, 0xff);
    CHECK(mh_ne2000_read(&n, 7, 1) == 0x80); /* reset isn't W1C */
    wr(&n, 14, 0x49);
    dma(&n, 0, 32, 0x0a);
    for (unsigned i = 0; i < 16; i++) {
        unsigned b = i < 6 ? mac[i] : i >= 14 ? 0x57 : 0;
        CHECK(mh_ne2000_read(&n, 0x10, 2) == b * 0x101);
    }
    CHECK(n.rbcr == 0 && (n.isr & 0x40));
    CHECK(!mh_ne2000_irq(&n));
    wr(&n, 15, 0x40); CHECK(mh_ne2000_irq(&n));
    wr(&n, 7, 0x40); CHECK(!mh_ne2000_irq(&n));
    CHECK(mh_ne2000_read(&n, 0x10, 2) == 0xffff && n.rbcr == 0);
    dma(&n, 0x4000, 16384, 0x12);
    for (unsigned i = 0; i < 8192; i++) mh_ne2000_write(&n, 0x10, 2, i ^ 0x5a5a);
    CHECK(n.rbcr == 0 && n.rsar == 0x8000);
    dma(&n, 0x4000, 16384, 0x0a);
    for (unsigned i = 0; i < 8192; i++) CHECK(mh_ne2000_read(&n, 0x10, 2) == (i ^ 0x5a5a));
    wr(&n, 1, 0x40); wr(&n, 2, 0x80);
    dma(&n, 0x7fff, 3, 0x12);
    mh_ne2000_write(&n, 0x10, 2, 0x1234);
    mh_ne2000_write(&n, 0x10, 2, 0x5678);
    CHECK(n.ram[0x3fff] == 0x34 && n.ram[0] == 0x12 && n.ram[1] == 0x78);
    CHECK(n.rbcr == 0 && n.rsar == 0x4002);
    wr(&n, 0, 0x62);
    for (unsigned i = 0; i < 6; i++) wr(&n, i + 1, mac[i]);
    wr(&n, 7, 0x46);
    CHECK(memcmp(n.par, mac, 6) == 0 && mh_ne2000_read(&n, 7, 1) == 0x46);
    wr(&n, 0, 0xa2);
    CHECK(mh_ne2000_read(&n, 1, 1) == 0x40);
    CHECK(mh_ne2000_read(&n, 2, 1) == 0x80);
    CHECK(!n.unsupported);

    mh_pccard c;
    mh_pccard_init(&c, 0, &mh_pccard_ne2000);
    mh_ne2000_init(&c.nic, mac);
    uint32_t value;
    CHECK(c.kind->read(&c, MH_PCCARD_WINDOW_A, 0, 1, &value) && value == 1);
    CHECK(!c.kind->read(&c, MH_PCCARD_WINDOW_A, 1, 1, &value));
    CHECK(!c.kind->read(&c, MH_PCCARD_WINDOW_B, 0x300, 1, &value));
    bool present = false;
    mh_pccard_port port = { .card = &c, .window = MH_PCCARD_WINDOW_A,
                            .present = &present };
    CHECK(mh_pccard_read(&port, 0, 1) == 0xff);
    mh_pccard_write(&port, 0x3f8, 1, 0x60);
    CHECK(c.config == 0);
    present = true;
    CHECK(mh_pccard_read(&port, 0, 1) == 1);
    CHECK(c.kind->write(&c, MH_PCCARD_WINDOW_A, 0x3f8, 1, 0x60));
    CHECK(c.kind->read(&c, MH_PCCARD_WINDOW_A, 0x300, 1, &value) && value == 0x21);
    CHECK(!c.kind->read(&c, MH_PCCARD_WINDOW_B, 0x300, 1, &value));
    CHECK(!c.kind->read(&c, MH_PCCARD_WINDOW_A, 0x320, 1, &value));
    ...
    for (unsigned at = 0; at < 200; at += 2) {
        if (!mh_ne2000_magic_attribute_byte(at, 0, &byte)) break;
        if (byte == 0xa0) { magic_at = at; break; }
    }
    CHECK(magic_at != 0);
    CHECK(mh_ne2000_magic_attribute_byte(magic_at + 2, 0, &byte) && byte == 32);
    uint8_t tuple[32];
    for (unsigned i = 0; i < 32; ++i)
        CHECK(mh_ne2000_magic_attribute_byte(magic_at + 4 + i * 2, 0, &tuple[i]));
    ...
    CHECK(mh_ne2000_card_attribute_byte(magic_at, 0, &byte) && byte == 0xff);
    /* Guest's BE EtherType store must retain Ethernet byte order in NIC RAM. */
    dma(&c.nic, 0x4000, 2, 0x12);
    CHECK(c.kind->write(&c, MH_PCCARD_WINDOW_A, 0x310, 2, 0x0806));
    CHECK(c.nic.ram[0] == 8 && c.nic.ram[1] == 6);
    dma(&c.nic, 0x4000, 2, 0x0a);
    CHECK(c.kind->read(&c, MH_PCCARD_WINDOW_A, 0x310, 2, &value) && value == 0x0806);
    wr(&c.nic, 0, 0x26);
    CHECK(c.nic.tx_requests == 1 && !(c.nic.isr & 2)); /* no fabricated TX success */
    CHECK(c.kind->write(&c, MH_PCCARD_WINDOW_A, 0x3f8, 1, 0x80));
    CHECK(!c.kind->read(&c, MH_PCCARD_WINDOW_B, 0x300, 1, &value));
    CHECK(!c.kind->read(&c, MH_PCCARD_WINDOW_A, 0x300, 1, &value));
