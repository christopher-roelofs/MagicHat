...
static void wr(tx39 *s,uint32_t reg,uint32_t val) { mrc_tx39_write(s,reg,4,val); }
static uint32_t rd(tx39 *s,uint32_t reg) { return mrc_tx39_read(s,reg,4); }
int main(void) {
    uint8_t ram[32]; memset(ram,0xcc,sizeof(ram));
    mrc_bus bus; mrc_bus_init(&bus); mrc_bus_add_ram(&bus,"ram",0x1000,ram,sizeof(ram),sizeof(ram));
    r3900 cpu={0}; cpu.bus=&bus; tx39 soc; mrc_tx39_init(&soc,&cpu,36864000);
    struct capture c={0}; tx39_mbus_port port={.ctx=&c,.command=command,.transmit=transmit,.input_high=true};
    mrc_mbus_connect(&soc.mbus,&port);
    CHECK(!mrc_mbus_receive_word(&soc.mbus,123));
    CHECK(!mrc_mbus_receive_command(&soc.mbus,123));
    ...
    CHECK(mrc_mbus_receive_word(&soc.mbus,0xabcd1234));
    CHECK(rd(&soc,TX39_MBUSPAYLOAD)==0x1234);
    CHECK(mrc_mbus_receive_command(&soc.mbus,0xc000));
    ...
    CHECK(mrc_mbus_receive_word(&soc.mbus,0x01020304));
    CHECK(rd(&soc,TX39_MBUSDMACOUNT)==4 && !(rd(&soc,TX39_INTRSTATUS(2))&0x20));
    CHECK(mrc_mbus_receive_word(&soc.mbus,0x05060708));
    CHECK(rd(&soc,TX39_INTRSTATUS(2))&0x10);
    CHECK(mrc_mbus_receive_word(&soc.mbus,0x090a0b0c));
    CHECK(rd(&soc,TX39_MBUSDMACOUNT)==8 && (rd(&soc,TX39_INTRSTATUS(2))&0x20));
    CHECK(!mrc_mbus_receive_word(&soc.mbus,0xdeadbeef));
    ...
    CHECK(!mrc_mbus_receive_word(&soc.mbus,0));
    wr(&soc,TX39_INTRCLEAR(2),INT2_MBUS_MASK);
    mrc_mbus_input(&soc.mbus,false); CHECK(rd(&soc,TX39_INTRSTATUS(2))&4);
    wr(&soc,TX39_INTRCLEAR(2),4); mrc_mbus_input(&soc.mbus,false); CHECK(!(rd(&soc,TX39_INTRSTATUS(2))&4));
    mrc_mbus_input(&soc.mbus,true); CHECK(rd(&soc,TX39_INTRSTATUS(2))&8);
