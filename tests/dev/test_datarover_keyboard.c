...
static tx39 soc;
static mrc_dr_keyboard keyboard;
static uint8_t ram[256];
static void wr(uint32_t off,uint32_t v) { CHECK(mrc_mbus_write(&soc.mbus,off,v)); }
...
static void read_packet(uint16_t command,unsigned length) {
    ...
    mrc_dr_keyboard_service(&keyboard);
    CHECK(!keyboard.pending_read);
}
int main(void) {
    mrc_bus bus; mrc_bus_init(&bus);
    mrc_bus_add_ram(&bus,"ram",0x1000,ram,sizeof(ram),sizeof(ram));
    r3900 cpu={0}; cpu.bus=&bus; mrc_tx39_init(&soc,&cpu,36864000);
    mrc_dr_keyboard_init(&keyboard,&soc.mbus);
    mrc_mbus_connect(&soc.mbus,&keyboard.port);
    ...
    mrc_dr_keyboard_service(&keyboard); CHECK(keyboard.pending_read==2);
    wr(TX39_MBUSCTRL,0x8ab); mrc_dr_keyboard_service(&keyboard);
    CHECK(soc.mbus.reg[(TX39_MBUSPAYLOAD-TX39_MBUS_FIRST)/4]==MRC_MBKEY_ID);
    ...
    for(unsigned i=0;i<14;++i)CHECK(mrc_dr_keyboard_key(&keyboard,0x1c,false,true));
    CHECK(mrc_dr_keyboard_key(&keyboard,0x75,true,false));
    CHECK(keyboard.port.input_high);
    ...
    CHECK(mrc_dr_keyboard_key(&keyboard,0x32,false,true));
    ...
    CHECK(mrc_dr_keyboard_key(&keyboard,0x12,false,true));
    ...
    CHECK(mrc_dr_keyboard_key(&keyboard,0x1c,false,true));
    cmd(0xcc24); wr(TX39_MBUSDMASTART,0x9000); wr(TX39_MBUSDMALENGTH,12);
    wr(TX39_MBUSCTRL,0x108ab); mrc_dr_keyboard_service(&keyboard);
    ...
    mrc_dr_keyboard_init(&keyboard,&soc.mbus);
    mrc_mbus_connect(&soc.mbus,&keyboard.port);
    CHECK(mrc_dr_keyboard_host_key(&keyboard,225,true,false)); // left Shift
    CHECK(mrc_dr_keyboard_host_key(&keyboard,5,true,false)); // B position
    CHECK(mrc_dr_keyboard_host_key(&keyboard,5,false,false));
    CHECK(mrc_dr_keyboard_host_key(&keyboard,225,false,false));
    uint8_t packet[16];
    CHECK(mrc_mb_keyboard_read(&keyboard.keys,packet,16)==7);
    CHECK(!memcmp(packet,(uint8_t[]){6,0x12,0x32,0xf0,0x32,0xf0,0x12},7));
    CHECK(!mrc_dr_keyboard_host_key(&keyboard,72,true,false)); // Pause unsupported
    CHECK(mrc_dr_keyboard_host_key(&keyboard,228,true,false)); // right Control E0
    // A full queue must not lose the release when the window loses focus.
    while(keyboard.keys.count<256) CHECK(mrc_dr_keyboard_key(&keyboard,0x1c,false,true));
    mrc_dr_keyboard_release(&keyboard);
    mrc_dr_keyboard_service(&keyboard);
    CHECK(keyboard.release[228/8]&(1u<<(228&7)));
    uint8_t all[256]; mrc_mb_keyboard_read(&keyboard.keys,all,256);
    mrc_dr_keyboard_service(&keyboard);
    CHECK(!keyboard.release[228/8]);
    CHECK(mrc_mb_keyboard_read(&keyboard.keys,packet,16)==5);
    ...
    CHECK(mrc_dr_keyboard_host_key(&keyboard,4,true,false));
    ...
    uint8_t state[MRC_DR_KEYBOARD_STATE_SIZE], roundtrip[MRC_DR_KEYBOARD_STATE_SIZE];
    mrc_dr_keyboard_encode(&keyboard,state);
    uint32_t irq=soc.icu.status[1];
    mrc_dr_keyboard_init(&keyboard,&soc.mbus);
    CHECK(mrc_dr_keyboard_decode(&keyboard,state));
    CHECK(soc.mbus_port==&keyboard.port && soc.icu.status[1]==irq);
    mrc_dr_keyboard_encode(&keyboard,roundtrip);
    CHECK(!memcmp(state,roundtrip,sizeof(state)));
    state[12]=2; CHECK(!mrc_dr_keyboard_decode(&keyboard,state));
    CHECK(keyboard.keys.count==1 && keyboard.pending_read==2);
    mrc_mbus_connect(&soc.mbus,NULL);
