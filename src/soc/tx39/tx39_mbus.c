...
void mrc_mbus_connect(tx39_mbus *m, tx39_mbus_port *port)
{
    ...
    if (high != old_high) mrc_icu_raise(&m->soc->icu, 2, high ? 8u : 4u);
}
...
void mrc_mbus_input(tx39_mbus *m, bool high)
{
    ...
    mrc_icu_raise(&m->soc->icu, 2, high ? 8u : 4u);
}
...
bool mrc_mbus_receive_command(tx39_mbus *m, uint16_t word)
{
    ...
    mrc_icu_raise(&m->soc->icu, 2, INT2_MBUSDET);
    return true;
}
...
bool mrc_mbus_receive_word(tx39_mbus *m, uint32_t word)
{
    ...
    if (ctrl & DMA_RX) {
        ...
        bool ok;
        mrc_bus_write(m->soc->cpu->bus, start + p->rx_bytes, 4, word, &ok);
        if (!ok) {
            p->rx_complete = true;
            mrc_icu_raise(&m->soc->icu, 2, INT2_MBUSRXERR);
            return false;
        }
        p->rx_bytes += 4;
...
static void transmit_dma(tx39_mbus *m)
{
    ...
    for (uint32_t at = 0; at < length; at += 4) {
        bool ok;
        uint32_t word = mrc_bus_read(m->soc->cpu->bus, start + at, 4, &ok);
        if (!ok) { mrc_icu_raise(&m->soc->icu, 2, INT2_MBUSTXERR); return; }
        m->reg[index_of(TX39_MBUSDMACOUNT)] = at;
        if (p->transmit) p->transmit(p->ctx, word, 32);
    }
    mrc_icu_raise(&m->soc->icu, 2, DMA_END | INT2_MBUSEMPTY | INT2_MBUSTXBUFAVAIL);
}
...
{
    mrc_icu_raise(&m->soc->icu, 2, INT2_MBUSTXBUFAVAIL | INT2_MBUSEMPTY);
}
