 ...
 * the host's stdout; host input is injected with mrc_uart_rx_byte().
 */
...
void mrc_uart_update(tx39_uart *u)
{
    ...
    while (t->shifting && now >= t->tx_done) {
        ...
        if (t->holding) {
            t->shift = t->hold;
            t->holding = false;
            t->tx_done += mrc_uart_frame_cycles(u);
            mrc_icu_raise(&u->soc->icu, 2,
...
bool mrc_uart_write(tx39_uart *u, uint32_t off, uint32_t val)
{
    ...
    case TX39_UART_CTRL1: {
        ...
        if (!was_enabled && (u->ctrl1 & UART_CTRL1_ENUART))
            ...
            mrc_icu_raise(&u->soc->icu, 2, tx_int_bits(u));
        mrc_uart_update(u);
        return true;
    }
    ...
    case TX39_UART_TXHOLD: {
        ...
        if (!t->shifting) {
            t->shift = (uint8_t)val;
            t->shifting = true;
            t->tx_done = u->soc->cpu->cycle_count + mrc_uart_frame_cycles(u);
            mrc_icu_raise(&u->soc->icu, 2,
                          u->index == 0 ? INT2_UARTATXINT : INT2_UARTBTXINT);
        } else {
            if (t->holding)
                mrc_icu_raise(&u->soc->icu, 2, u->index == 0 ?
...
void mrc_uart_rx_byte(tx39_uart *u, uint8_t byte)
{
    ...
    u->rx = byte;
    u->rx_full = true;
    mrc_icu_raise(&u->soc->icu, 2, rx_int_bit(u));
}
