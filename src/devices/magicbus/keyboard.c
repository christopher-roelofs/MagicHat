...
bool mrc_mb_keyboard_key(mrc_mb_keyboard *k, uint8_t code, bool extended, bool down)
{
    ...
    if (n > MRC_MBKEY_CAPACITY - k->count) {
        k->dropped_events++;
        return false;
    }
    ...
    for (unsigned j = 0; j < n; ++j)
        k->queue[(k->head + k->count++) % MRC_MBKEY_CAPACITY] = bytes[j];
    return true;
}
...
size_t mrc_mb_keyboard_read(mrc_mb_keyboard *k, uint8_t *data, size_t capacity)
{
    ...
    for (size_t i = 0; i < n; ++i)
        data[i + 1] = k->queue[(k->head + i) % MRC_MBKEY_CAPACITY];
    k->head = (k->head + n) % MRC_MBKEY_CAPACITY;
    k->count -= n;
    return n + 1;
}
