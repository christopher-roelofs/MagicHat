...
static void build(mrc_ui *ui)
{
     ...
     */
    if (link) snprintf(status, sizeof(status), "%s", mrc_pclink_message(link));
    if (status[0])
        rows[row_count++] = (mrc_ui_row){
            .kind = MRC_UI_ROW_HEADING, .label = status,
        };
    if (link) {
        uint32_t total = mrc_pclink_total(link);
        uint32_t sent = mrc_pclink_sent(link);
        snprintf(progress, sizeof(progress), "%u of %u bytes", sent, total);
        rows[row_count++] = (mrc_ui_row){
            .kind = MRC_UI_ROW_CHOICE, .label = "Sent", .value = progress,
        };
    }
    rows[row_count++] = (mrc_ui_row){
    ...
    mrc_ui_open_panel(ui, "Install", rows, row_count);
    showing = true;
}
...
{
    mrc_import_cancel(MRC_IMPORT_PACKAGE);
    if (picker) {
...
void mrc_install_panel_tick(mrc_ui *ui, mrc_runtime *m)
{
    if (showing && !picker) {
        ...
        if (mrc_import_result(MRC_IMPORT_PACKAGE, path, sizeof(path), error, sizeof(error))) {
            notice[0] = 0;
    ...
    static mrc_pclink_state was = MRC_PCLINK_FAILED;
    mrc_pclink_state now = mrc_pclink_state_of(link);
...
bool mrc_install_panel_row(mrc_ui *ui, mrc_runtime *m, int id)
{
    ...
    if (id == ROW_CHOOSE) {
        ...
        if (mrc_import_available()) {
            snprintf(notice, sizeof(notice), "%s", mrc_import_request(MRC_IMPORT_PACKAGE)
        ...
        picker = mrc_picker_open(ui, "Choose a package",
