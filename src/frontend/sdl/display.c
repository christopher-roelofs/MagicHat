                         ...
                         int width, int height, uint8_t background)
{
    ...
    const char *rotation = SDL_getenv("MH_DISPLAY_ROTATION");
    if (rotation && SDL_strcmp(rotation, "0") && !mh_sdl_rotation()) {
        SDL_SetError("MH_DISPLAY_ROTATION must be 0, 90, 180 or 270");
        return false;
    }
    if (mh_sdl_sideways()) { int tmp=width; width=height; height=tmp; }
    const char *fullscreen = SDL_getenv("MH_FULLSCREEN");
    ...
    if (!d->renderer) { mh_sdl_display_close(d); return false; }
                             ...
                             unsigned panel_height, bool integer_scale)
{
    ...
    SDL_Rect dst;
    mh_sdl_panel_rect(d->renderer, panel_width, panel_height, integer_scale,
                       mh_ui_inset_left(d->ui), mh_ui_inset_right(d->ui),
                       &dst);
    ...
    if (SDL_UpdateTexture(d->texture, NULL, d->pixels, (int)(d->width * 4)) != 0 ||
        ...
        SDL_RenderCopyExF(d->renderer, d->texture, NULL, &copy,
                         mh_sdl_rotation(), NULL, SDL_FLIP_NONE) != 0)
        return false;
    mh_ui_draw(d->ui);
...
{
    return d ? mh_ui_rail_width(d->ui) : 0;
}
                          ...
                          mh_frame_format format)
{
    for (unsigned i = 0; i < count; i++) {
        uint32_t v = format == MH_FRAME_LCD2 ? 255 - (in[i] & 3) * 85 : in[i];
...
{
    return m && m->ops->save_state && mh_state_path();
}

bool mh_sdl_save_state(mh_runtime *m)
{
    const char *path = mh_state_path();
...
bool mh_sdl_display_event(mh_sdl_display *d, const SDL_Event *e,
                           mh_runtime *m, Uint64 *wall_ref)
{
    if (e->type == SDL_APP_WILLENTERBACKGROUND) {
        if (!d->backgrounded) {
            ...
            if (mh_sdl_can_save_state(m)) mh_sdl_save_state(m);
        }
        return true;
    }
