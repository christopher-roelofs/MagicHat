...
void mh_gui68k_set_host_battery(bool enabled) { host_battery_enabled = enabled; }
void mh_gui68k_set_audio(bool enabled) { audio_enabled = enabled; }
 ...
 * neither of them.
 */
static void mh_gui_action(mh_runtime *m, int action, mh_sdl_display *d)
{
    ...
    case MH_UI_ICON_OPTION:
        ...
        if (m->ops->option && m->ops->option_held) {
            ...
            mh_ui_set_lit(g_ui, MH_UI_ICON_OPTION, now);
        }
        break;
...
{
    mh_sdl_panel_rect(renderer, PIC2000_SCREEN_W, PIC2000_SCREEN_H,
                       mh_gui_integer,
...
static bool panel_point(int ex, int ey, unsigned *px, unsigned *py)
{
    ...
    return mh_sdl_panel_point(&dst, x, y, PIC2000_SCREEN_W,
...
bool mh_gui68k_available(void) { return true; }

static int pic2000_present(mh_runtime *m, uint64_t insns_total,
                           struct mh_shell *sh)
{
    /*
    ...
    const char *title = !strcmp(m->machine_id, "envoy") ?
        "MagicHat — Motorola Envoy" : !strcmp(m->machine_id, "hix300") ?
        "MagicHat — Sony HIX-300" : "MagicHat — Sony PIC-2000";
    if (!mh_shell_attach(sh, title, PIC2000_SCREEN_W, PIC2000_SCREEN_H))
        return 1;
    ...
    while (running && !mh_state_device_requested() &&
                ...
                if (power.has_battery && power.percent >= 0 && m->ops->host_battery) {
                    ...
                    if (!battery_reported) {
                        fprintf(stderr, "%s: host battery %d%% via %s; %s\n",
                                m->machine_id, power.percent, mh_host_power_source(),
        ...
        while (SDL_PollEvent(&event)) {
            if (mh_card_panel_event(g_ui, m, &event)) continue;
            ...
            if (mh_ui_event(g_ui, &event)) {
                mh_gui_action(m, mh_ui_take_action(g_ui), display);
            ...
            case SDL_MOUSEBUTTONDOWN:
                if (event.button.button == SDL_BUTTON_LEFT &&
                    ...
                    if (panel_point(event.button.x, event.button.y, &px, &py)) {
                        mouse_down = true;
                        if (!mh_pen_enqueue(&pen_events, m->ops->slots(m->board),
            ...
            case SDL_MOUSEMOTION:
                if (mouse_down) {
                    ...
                    if (panel_point(event.motion.x, event.motion.y, &px, &py) &&
                        !mh_pen_enqueue(&pen_events, m->ops->slots(m->board),
                            m->nominal_slots_hz, PEN_HOLD_INSNS,
        ...
        const uint64_t target = mh_sdl_owed_slots(now, wall_ref, perf_hz, m->nominal_slots_hz);
        ...
        for (;;) {
            ...
            while (pen_events.head && pen_events.head->at <= current) {
                ...
                if (!mh_pen_deliver(&pen_events, current,
                        m->nominal_slots_hz, &delivered)) break;
                const mh_pen_event *e = &delivered;
                    ...
                    frame_w == PIC2000_SCREEN_W && frame_h == PIC2000_SCREEN_H;
        {
            ...
            panel_rect(&dst);
            mh_lcd_frame view;
            if (!mh_lcd_render(lcd, display, have ? pixels : NULL,
                                PIC2000_SCREEN_W, PIC2000_SCREEN_H,
                                m->frame_format,
                                (unsigned)(mh_sdl_sideways() ? dst.h : dst.w),
                                &view)) {
        ...
        if (!mh_sdl_display_present(display, PIC2000_SCREEN_W,
                                     PIC2000_SCREEN_H, mh_gui_integer)) {
     ...
     * one's -- the install panel keeps this machine's own mh_runtime, and
...
bool mh_gui68k_available(void) { return false; }
static int pic2000_present(mh_runtime *m, uint64_t insns_total,
                           struct mh_shell *sh)
{
    (void)m; (void)insns_total; (void)sh;
    fprintf(stderr, "mhat: SDL2 support was not built\n");
    return 1;
}
 ...
 */
int mh_gui68k_run(m68k_machine *board, uint64_t slots, struct mh_shell *shell)
{
    mh_runtime view = mh_pic2000_runtime(board);
