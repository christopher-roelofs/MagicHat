#ifndef MH_RUNTIME_MACHINE_H
#define MH_RUNTIME_MACHINE_H
#include <stdbool.h>
#include <stdint.h>
 ...
 */
typedef void (*mh_runtime_audio_sink)(void *, int16_t);
typedef struct mh_runtime_ops {
    ...
    void (*audio_sink)(void *, mh_runtime_audio_sink, void *);
     ...
     * implementation.
     */
    struct mh_pclink *(*install)(void *, const char *path);
...
typedef enum { MH_FRAME_GRAY8, MH_FRAME_LCD2 } mh_frame_format;
typedef struct {
    void *board;
    const char *machine_id;
    uint32_t nominal_slots_hz;
    const mh_runtime_ops *ops;
    mh_frame_format frame_format;
} mh_runtime;
 ...
 */
static inline uint64_t mh_time_ns(uint64_t cycles, uint32_t hz) {
